...
import { UserPlus, User, KeyRound, Mail, Sun, Moon } from 'lucide-react';
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        showToast('success', 'Registration successful! Welcome to ContactHub.');
        navigate('/');
      }
    } catch (err: any) {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2 mb-8">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-950 dark:text-slate-50">
            Create ContactHub Account
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Join the smart way to orchestrate your contacts network.
          </p>
        </div>
        ...
        <div className="glass-card p-8 shadow-xl relative overflow-hidden">
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
            <div className="relative">
              <User className="absolute right-3.5 top-9.5 h-4 w-4 text-slate-400 pointer-events-none" />
              <Input
                label="Full Name"
